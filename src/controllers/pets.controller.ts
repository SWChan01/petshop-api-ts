...
export const createPet = (req: Request, res: Response) => {
    const {name, race, type, customer_id} = req.body;
        try{
        db.query(`SELECT * FROM customers WHERE customers.id = ?`, [customer_id], (err, customerRows) => {

            // If there's any error
            if(err) return res.status(500).json({Message: 'INTERNAL_ERROR', Error: err})
            // If there's no customer with the provided ID
            if(customerRows.length === 0) return res.status(404).json({Message: 'CUSTOMER_NOT_FOUND'});

            // Check if the customer already has not a pet with the same name
            db.query('SELECT * FROM pets WHERE name = ? AND customer_id = ?', [name, customer_id], (err, petRows) => {
                if(err) return res.status(500).json({Message: 'INTERNAL_ERROR', Error: err});
                if(petRows.length > 0) return res.status(409).json({ Message: 'PET_ALREADY_EXISTS' });
...
export const detailPet = (req: Request, res: Response) => {
    ...
    try {
        db.query('SELECT * FROM pets WHERE pets.id = ?', [petId], (err, result) => {
            if(err) return res.status(500).json({Message: 'INTERNAL_ERROR', Error: err});
            if(result.length === 0) return res.status(404).json({Message: 'PET_NOT_FOUND', Error: err});
