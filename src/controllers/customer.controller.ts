...
export const createCustomer = async (req: Request, res: Response) => {
    ...
    try {
        ...
        db.query('SELECT * FROM customers WHERE customers.email = ?', [email], (err, customerRow) => {
            if(err) return res.status(500).json({Message: 'INTERNAL_ERROR', Error: err});
            if(customerRow.length > 0) return res.status(409).json({Message: 'CUSTOMER_ALREADY_EXISTS'});
...
export const updateCustomer = async (req: Request, res: Response) => {
    ...
    try {
        ...
        db.query('SELECT * FROM customers WHERE id = ?', [id], (err, row) => {
            if(err) return res.status(500).json({Message: 'INTERNAL_ERROR', Error: err});
            if(row.length === 0)return res.status(404).json({Message: 'CUSTOMER_NOT_FOUND', Error: err});
            ...
            db.query(`UPDATE customers SET ${setClause} WHERE id = ?`, [...values, id], (err, result) => {
                if(err) return res.status(500).json({Message: 'INTERNAL_ERROR', Error: err});
...
export const deleteCustomer = async (req: Request, res: Response) => {
    ...
    try {
        db.query('SELECT * FROM customers WHERE customers.id = ?', [customerId], async (err, custRow) => {
            if (err) return res.status(500).json({Message: 'INTERNAL_ERROR', Error: err});
            if(custRow.length === 0) return res.status(404).json({Message: 'CUSTOMER_NOT_FOUND'});
